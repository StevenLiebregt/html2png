$(function () {
    ...
    // Thanks to the following script from `floribon` on stackoverflow // https://stackoverflow.com/questions/27992992/i-need-list-of-all-class-name-of-font-awesome
    // Run on cheatsheet page from fontawesome in developer console
    //
    //     var names = [];
    ...
    function toggleBootstrapCss() {
        ...
        hasBootstrapCss = !hasBootstrapCss;

        updatePreview();
    }

    function toggleFontAwesomeCss() {
        $('#toggle-font-awesome-css')
            .toggleClass('btn-default')
            .toggleClass('btn-success');

        hasFontAwesomeCss = !hasFontAwesomeCss;
    ...
    function insertFontAwesomeIcon() {
        var content = '<i class="fa ' + $(this).data('icon') + '"></i>';
        ...
        $('#font-awesome-modal').modal('hide');
    }

    function showDeleteCssModal() {
        $('#delete-css-modal').modal();
    }
    ...
    function showDeleteHtmlModal() {
        $('#delete-html-modal').modal();
    }
    ...
    $('#delete-css').on('click', showDeleteCssModal);
    $('#confirm-delete-css').on('click', deleteCss);

    $('#insert-table').on('click', insertTableHtml);
    $('#insert-font-awesome-icon').on('click', showFontAwesomeModal);
    $('#delete-html').on('click', showDeleteHtmlModal);
    $('#confirm-delete-html').on('click', deleteHtml);
    ...
    function updatePreview() {
        ...
        if (hasBootstrapCss) {
            $('head', preview).append('<link rel="stylesheet" href="css/bootstrap.min.css">');
        }
        if (hasFontAwesomeCss) {
            $('head', preview).append('<link rel="stylesheet" href="css/font-awesome.min.css">');
        }
    ...
    function changePreviewDimensions() {
        var width = $('#preview-width').val();
        var height = $('#preview-height').val();

        $previewWindow.width(width);
        $previewWindow.height(height);
    }

    $('#preview-width').on('keyup', changePreviewDimensions);
    $('#preview-height').on('keyup', changePreviewDimensions);
